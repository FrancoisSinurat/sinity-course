...
export default function EnrollRecommendationsList() {
  ...
  return (
    ...
    //     <Card key={course.course_id_int}>
    //       <CardContent className="p-4 space-y-1">
    //         <h3 className="text-lg font-semibold">{course.name}</h3>
    //         <div className="text-sm text-muted-foreground flex items-center gap-1">
    //           <Star className="h-4 w-4 text-yellow-500 fill-yellow-500" />
    //           {course.average_rating.toFixed(1)} ({course.total_reviewers})
    //         </div>
    //       </CardContent>
    //     </Card>
    //   ))}
    // </div>
    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4 mt-4 w-full">
          {recommendedCourses
